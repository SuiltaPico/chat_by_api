...
`,
  },
  {
    ...
    content: `
### Added
* 当对话记录正在生成时，其左侧栏的对话记录选择卡的图标会有动画提示。
* 对话暂停生成。
* 对话继续生成。
* 最后一个用户消息会有快捷发送对话按钮。
* Gecko 内核浏览器使用警告。
### Changed
* 优化了Markdown内联、块的代码块的字间距和行高样式。
* 优化了消息编辑器的样式。
### Fixed
* 修复了当前发送的对话记录 Token 超过上限时，错误提示错误显示 API-KEY 无效的情况。
* 修复了错误的数据库写入导致无法打开页面的问题。
* 修复了切换了无法请求模型列表的 api 时, 模型列表仍然保留上一次获取的列表的问题。
* 修复了当前状态不显示的问题。
`,
  },
  {
    ...
    content: `
### Added
* Markdown 在渲染时会在没有大小写区分的字符（如中文）和具有大小写区分的字符（如abc）之间自动添加空格。
* 应用初始化时会根据 API-KEY 自动获取可用模型。
### Changed
* 优化了Markdown渲染后的消息样式。
* 优化了全局页面的文字表现。
* 优化了 API-KEY 管理的界面。
* 优化了左侧栏选择卡当前页面与选择卡打开界面不同时的当前页面指示。
### Fixed
* 修复了Markdown渲染后错误的内联代码块样式。
* 修复了在 \`/chat\` 页面下，按下发送消息按钮后出现的卡顿问题。
* 修复了消息角色不能切换的问题
* 修复了在任意页面下，左侧栏选择卡在页面打开时总会跳转到 chat 选择卡的问题。
`,
  },
  {
    version: "2.8.0",
...
### Changed
* 优化了移动端的界面。
* 在宽度达到一定程度，模型选项会出现在工具栏内。
* 对话记录栏的自动根据最近更改时间排序功能恢复正常。
### Fixed
* 数据更改（删除消息、删除对话记录、对话生成、对话编辑等）的更新不及时导致的文档冲突、写入错误、行为不正常等情况。通常表现为界面冻结。
`,
  },
  {
    version: "2.6.0",
...
* 对话记录删除前确认。
### Changed
* 优化了对话记录的信息，现在能看到更详尽的信息。
* API-KEY 管理的向上移动按钮改成了置顶按钮。
`,
  },
  {
    version: "2.5.1",
